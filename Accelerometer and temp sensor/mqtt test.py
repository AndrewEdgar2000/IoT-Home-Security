...
while True:
    ...
    if(outputX < -0.50 or outputX > 0.50):
        ...
        alert = True
        client.publish("acceleration","Warning Concerning movement!!")
        time.sleep(5)
    elif(outputX < -0.30 or outputX > 0.30):
            print("someone there?")
            client.publish("acceleration" ,"slight sudden movement")
            time.sleep(5)
    elif(outputX < -0.05 or outputX > 0.05):
            print("hmm must have been the wind")
            client.publish("acceleration","slight movement")
            time.sleep(5)

    temperature = temps.gettemperature()
    humidity = temps.gethumidity()
    pressure = temps.getpressure()
